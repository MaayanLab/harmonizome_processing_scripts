...
gene_atb.cm = cminit(19795, 57, [], 'GeneSym', [], 'EnsembleAcc', [], 'GeneID', [], 'Tissue', [], 'TissueID', [], [], []);
...
fid = fopen('input/Ensembl_v65.Gencode_v10.ENSG.gene_info', 'r');
for i = 1:1:numprobes
    currline = fgetl(fid);
...
% read data
fid = fopen('input/57epigenomes.RPKM.pc', 'r');
currline = fgetl(fid);
currcells = strsplit(currline, '\t', 'CollapseDelimiters', false);
gene_atb.cm.entrydesc = currcells(2:end)';
...
fclose(fid);
gene_atb.cm.matrix = dlmread('input/57epigenomes.RPKM.pc', '\t', 1, 1);
gene_atb.cm.matrix(:,end) = []; % because of extra tab at end of lines

% map tissues to tissue ids
[o1, o2] = ismember(gene_atb.cm.entrydesc, tissueid);
o2(o2 == 0) = [];
gene_atb.cm.entry(o1) = tissue(o2);
