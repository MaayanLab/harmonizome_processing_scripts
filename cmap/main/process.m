...
atb_gene.uplists = listsinit(6100, [], 'Drug Name-Perturbation ID', [], [], [], [], [], 'GeneSym', [], [], [], [], false, []);
atb_gene.dnlists = listsinit(6100, [], 'Drug Name-Perturbation ID', [], [], [], [], [], 'GeneSym', [], [], [], [], false, []);


% read data
fid = fopen('input/CMAP_up.txt', 'r');
...
for i = 1:1:atb_gene.uplists.numterms
    ...
    atb_gene.uplists.term{i} = currcells{1};
%     atb_gene.uplists.termdesc{i} = currcells{2};
    atb_gene.uplists.entries{i} = currcells(3:end-1)'; % every line has trailing empty cell
...
fid = fopen('input/CMAP_down.txt', 'r');
...
for i = 1:1:atb_gene.dnlists.numterms
    ...
    atb_gene.dnlists.term{i} = currcells{1};
%     atb_gene.dnlists.termdesc{i} = currcells{2};
    atb_gene.dnlists.entries{i} = currcells(3:end-1)'; % every line has trailing empty cell
