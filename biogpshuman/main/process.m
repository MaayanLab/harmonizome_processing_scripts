...
gene_atb.cm = cminit(44775, 84, [], 'GeneSym', [], 'ProbeID', [], 'GeneID', [], 'Tissue', [], [], [], [], []);
...
fid = fopen('input/gnf1h.annot2007.tsv', 'r');
currline = fgetl(fid);
for i = 1:1:numprobes
    currline = fgetl(fid);
...
numprobes = 22283;
probeid2 = cell(numprobes, 1);
genesym2 = cell(numprobes, 1);
fid = fopen('input/GPL96-15653.txt', 'r');
for i = 1:1:17
    currline = fgetl(fid);
end
for i = 1:1:numprobes
    currline = fgetl(fid);
    currcells = strsplit(currline, '\t', 'CollapseDelimiters', false);
    probeid2{i} = currcells{1};
    genesym2{i} = currcells{11};
end
fclose(fid);
discard = strcmp(genesym2, '');
probeid2(discard) = [];
genesym2(discard) = [];

probeid = [probeid; probeid2];
genesym = [genesym; genesym2];


% read data
fid = fopen('input/U133AGNF1B.gcrma.avg.csv', 'r');
currline = fgetl(fid);
currcells = strsplit(currline, ',', 'CollapseDelimiters', false);
gene_atb.cm.entry = currcells(2:end)';
for i = 1:1:gene_atb.cm.numterms
    currline = fgetl(fid);
    idx = find(currline==',', 1, 'first');
    gene_atb.cm.termdesc{i} = currline(1:idx-1);
end
fclose(fid);
gene_atb.cm.matrix = dlmread('input/U133AGNF1B.gcrma.avg.csv', ',', 1, 1);
