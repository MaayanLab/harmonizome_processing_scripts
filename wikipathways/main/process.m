...
atb_gene.lists = listsinit(1318, [], 'Pathway', [], 'Wikipathways ID', [], [], [], 'GeneID', [], [], [], 'GeneID', false, []);
species = cell(atb_gene.lists.numterms, 1);

% read data
fid = fopen('input/dataset_20160407_original.gmt', 'r');
for i = 1:1:atb_gene.lists.numterms
    currline = fgetl(fid);
    currcells = strsplitbyadr(currline, '\t');
    subcells = strsplitbyadr(currcells{1}, '%');
    species{i} = strtrim(subcells{4});
    if strcmp(species{i}, 'Homo sapiens')
        atb_gene.lists.term{i} = strtrim(subcells{1});
        atb_gene.lists.termdesc{i} = strtrim(subcells{3});
        atb_gene.lists.entries{i} = cellfun(@strtrim, currcells(3:end)', 'uniformoutput', false);
        atb_gene.lists.entryids{i} = str2double(atb_gene.lists.entries{i});
        atb_gene.lists.numentries(i) = numel(atb_gene.lists.entries{i});
    end
end
fclose(fid);
discard = atb_gene.lists.numentries == 0 | cellfun(@isempty, atb_gene.lists.term) | strcmp(atb_gene.lists.term, '') | cellfun(@isempty, atb_gene.lists.termdesc) | strcmp(atb_gene.lists.termdesc, '');
atb_gene.lists = listsdiscard(atb_gene.lists, discard);
clear species fid ans currcells currline subcells discard i;
