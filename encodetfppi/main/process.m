...
gene_atb.cm = cmcoldiscard(gene_atb.cm, ishm);
...
% 34 unmapped out of 863 (4%) but there are duplicates in these numbers
...
[uatb, ~, ri] = unique(gene_atb.cm.entry);
catb = zeros(numel(uatb), 1);
for i = 1:1:numel(uatb)
    catb(i) = sum(ri==i);
end
if numel(unique(gene_atb.cm.entry)) < gene_atb.cm.numentries
...
[~, o2] = ismember(gene_atb.cm.entry, uatb);
o2(o2 == 0) = [];
uatb = uatb(o2);
catb = catb(o2);
targetsetsize = 750;
numt = zeros(gene_atb.cm.numentries, 1);
tval = zeros(gene_atb.cm.numentries, gene_atb.cm.numterms);
tidx = zeros(gene_atb.cm.numentries, 1);
for i = 1:1:gene_atb.cm.numentries
    t = unique(gene_atb.cm.matrix(:,i));
    numt(i) = numel(t);
    tval(i,1:numt(i)) = t';
    numg = sum(bsxfun(@gt, gene_atb.cm.matrix(:,i), t'), 1);
    d2 = (numg - targetsetsize).^2;
    tidx(i) = mean(find(d2 == min(d2)));
end
[ucatb, ui, ri] = unique(catb);
utidx = zeros(numel(ucatb), 1);
for i = 1:1:numel(ucatb)
    utidx(i) = nanmedian(tidx(ri==i));
end
figure(3);
clf;
plot(ucatb, utidx, '-ok');
...
% % thresholdindex = round(10.^(polyval(p, log10(numprofiles))));
% thresholdindex = round(10.^(spline(log10(x),log10(y),log10(numprofiles))));
% % % % thresholdindex = round(10.^(csaps(log10(x),log10(y),0.99,log10(numprofiles))));
thresholdindex = round(csaps(x,y,0.01,numprofiles));
% % % sp = spaps(log10(x), log10(y), 0);
% % % thresholdindex = round(10.^(fnval(sp, log10(numprofiles))));
thresholdindex(thresholdindex<1) = 1;
figure(3);
hold on;
plot(numprofiles, thresholdindex, 'sr');
hold off;
axis([0 125 0 25000]);
threshold = zeros(gene_atb.cm.numentries, 1);
for i = 1:1:gene_atb.cm.numentries
    threshold(i) = tval(i,thresholdindex(i));
...
figure(6); clf; hist(nanmax(gene_atb.cm.matrix, [], 2), 100); xlabel('rowmax');
figure(7); clf; hist(nanmax(gene_atb.cm.matrix, [], 1), 100); xlabel('colmax');
